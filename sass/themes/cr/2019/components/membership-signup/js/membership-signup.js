  ...
  $(document).ready(function() {
    ...
    $(".paragraph--membership-signup .select-amount-btn").click(function(e) {
      ...
      if ($(this).hasClass("active")) {
        ...
        var thisID = $thisBtn
          .closest(".paragraph--membership-signup")
          .attr("id");
        ...
        var $thisBtnParent = $thisBtn.parents(
          ".membership-signup__wrapper-copy--form-money"
        );
        var descriptionCopies = $thisBtnParent
          .find(".donation-copy")
          .children();
        ...
        var moneyBuySelectedValue = getMoneyBuyValue(
          $thisForm.find(".select-amount-btn.active")
        );
    ...
    $(".paragraph--membership-signup input[name='membership_amount']").on(
      ...
      function(event) {
        ...
        if ($thisForm.find(".select-amount-btn").hasClass("active")) {
          $thisForm.find(".select-amount-btn").removeClass("active");
          $thisForm
            .find(".money-buy--description")
            .removeClass("show-money-buy-copy");
        ...
        if (event.type === "input" || event.type === "click") {
          if (validateAmount(floatedAmound)) {
            ...
            if (moneyBuyRows[id].indexOf(amount.toString()) > -1) {
              ...
              $thisForm
                .find(".select-amount-btn.money-box--" + index)
                .addClass("active");
              var descriptionCopies = $thisForm
                .find(".donation-copy")
                .children();
              ...
              $thisForm
                .find(".other-description")
                .addClass("show-money-buy-copy");
    ...
    $(".paragraph--membership-signup select").on(
      "selectmenuchange input propertychange click",
      function() {
        var $thisSelect = $(this);
        ...
        if (givingType.toLowerCase() !== "MONTHLY".toLowerCase()) {
          ...
          $thisSelect
            .closest(".membership-signup__wrapper-copy--form-money")
    ...
    $(".paragraph--membership-signup input[name='membership_amount']").keypress(
      function(e) {
        var $thisInput = $(this);
        if (e.which == 13) {
          e.preventDefault();
          ...
          var thisID = $thisForm
            .parents(".paragraph--membership-signup")
            .attr("id");
    ...
    $(".paragraph--membership-signup .membership--submit").click(function(e) {
      ...
      var moneyBuySelected = getMoneyBuyValue(
        $thisForm.find(".select-amount-btn.active")
      );
    ...
    function setFormDefaults($thisParagraph, i) {
      ...
      if (colour) {
        $(".img-shadow", $newParagraphWithId).append(
          "<style> " +
            "#" +
            thisID +
            " .img-shadow" +
      ...
      var amount = parseFloat(
        $newParagraphWithId
          .find(".select-amount-btn.active")
          .text()
          .replace(/\D/g, "")
      );

      $newParagraphWithId.attr("data-current-amount", amount);
      var position = $newParagraphWithId
        .find(".select-amount-btn.active")
        .data("position");

      /* Add money buy description && currency */
      var descriptionCopies = $newParagraphWithId
        .find(".donation-copy")
        .children();
      ...
      if (
        url_string.indexOf("&amount=") > -1 &&
        amountValue.length > 0 &&
        rowIDValue.length &&
        url_string.indexOf("&rowID=") > -1
      ) {
        $("#" + rowIDValue)
      ...
      allRows[thisID] = {
        giving_type:
          givingType == "MONTHLY" ? "membership-payment" : "single-payment",
        cart_id: cartID,
        client_id: clientID,
    ...
    function setCurrentDataAmount(selector, amount) {
      selector
        .parents(".paragraph--membership-signup")
        .attr("data-current-amount", amount);
    }
    ...
    function nextStepHandler(
      ...
      /* Affiliate value */
      var affiliateValue = getQueryString("affiliate", url_string)
        ? getQueryString("affiliate", url_string)
        : "generic";
    ...
    function dataLayer_init($element, thisID) {
      ...
      for (i = 1; i < thisParagraph["buttons"].length; i++) {
        var thisAmount = parseFloat(
          parseFloat(thisParagraph["buttons"][i]["amount"]).toFixed(2)
        );
    ...
    function checkCookie() {
      var checkCookieValues = (document.cookie.match(
        /^(?:.*;)?\s*mship-previous-amount\s*=\s*([^;]+)(?:.*)?$/
      ) || [, null])[1];
